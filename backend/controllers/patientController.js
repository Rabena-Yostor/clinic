...
const getPatient = async (req, res) => {
  ...
  const { name } = req.body;
  const specificPatient = await patient.find({
...
const filterAppointment = async (req, res) => {
  ...
  const specificPatient = await patient.find({
    Appointment_Status: { $regex: new RegExp(status, "i") },
  });
...
const addFamilyMember = async (req, res) => {
  ...
  try {
    const workout = await familyMemberModel.create({
      Name,
      National_id,
      age,
      gender,
...
// New function to update patient appointments
